...
> #### 나쁜 코드를 피하기위해 체크해봐야할 리스트를 정리합니다.
>
> #### <a href = "SelfCheckList.md">셀프 체크리스트<a> 
...
- ### 클래스
  ...
  - ##### 상속 문제
    ...
    - 하위 클래스가 적절하지 않아서 되돌릴때 많은 비용이 든다.
    - 상위 클래스를 이해해야 하위 클래스를 이해할 수 있다.
    - 하위 클래스의 구현이 상위클래스의 구현에 의존하는 경우, 상위 클래스의 구현 변경이 자유롭지 못하다.LL
...
- #### `StoryBoard` vs `nib` vs `Code`
  ...
  - **StoryBoard**
    - ❇️ 스크린들을 표현하고 스크린들간의 연관 관계를 표현할수 있다.
    - ❇️ 씬들의 순서를 표현할 수있다.
    - ❇️  스토리보드는 전체가 아니라 해당 씬이 요구될 때 메모리에 로드되므로 전체가 로드되는 Nib보다 효율적일수있다.
    - ⚠️ 사용자정의 컨트롤이나 뷰를 보여줄때는 분리해서 표현하게 좋으므로 적절한 선택은 아닐 것이다.
    - ⚠️ 너무 씬이 많으면`prepareForSegue:sender:` 의 복잡도가 높아지므로 잘 관리하자

  <br>

  - **Nib**

    - ❇️ 모듈화를 통한 복잡도 감소의 이점이 있다.

    - ❇️ 반복적으로 여러 스크린에서 사용되는 경우

    - ⚠️❗️화면에서 어떤 역할인지 관계를 표현할 수는 없음

      <br>

  - **Code**

    - ❇️ 머지 충돌이 일어나기 좋은 환경일 때
    - ❇️ 동적으로 자꾸 변하는 레이아웃을 구성해야할 경우
    - ❇️ 레이어를 건들어야하는 경우( 그림자나 둥근모서리 )
    - 
...
- #### `struct` vs `class`
  ...
  - **struct**
    - ❇️ 객체들사이의 참조 관계를 단순화하는 것 충분히 가능할 때
    - ⚠️ 만약 멤버로 class 타입을 써야한다면 값이라는 정의에 어긋날 수 있다. 
  - **class**
    - ❇️ 데이터의 양이 복사하기에는 좀 크지 않은가?  그런 경우는 복사보다는 참조가 나을수있다. COW를 취하지 않는 경우
    -  ❇️ 공유자원으로서의 역할을 하는 객체일 때
...
- #### `if let`  vs `guard let` 
  ...
  - **guard**
    ...
    - ❇️ 한 방향으로의 의도가 존재하고 실패했을 때 하는 일을 명시

    - ❇️ guard 뭔가 성공해야 한다는 걸 좀 더 명시 할 수 있다.

    - ❇️ else 는 의도와 다른 일을 할 때 해야할 일 , 즉 실패에 대한 핸들링이다
  ...
  - **if let**
    ...
    - ❇️ 양방향 모두 동등한 무게를 가질 때 ,  즉 단순한 2가지 혹은 그 이상의 분기를 표현한다.
    - ❇️ else 는 if와 다른 조건일 때, 즉 다른 경우에 대한 핸들링 이다.
    - ❇️ 둘 다 정답이 될 수 있는 맞는 경우이다.
...
- #### `lazy property`  vs `eager property`
  ...
  - ❇️ 해당 프로퍼티의 사용이 무조건적이지 않다면  `lazy`를 고민해보자
  - ❇️ 객체를 생성하는 데 걸리는 오버헤드가 존재한다.  **객체가 쓰일 상황에 즉각적인 응답을 원한다면  `lazy`를 사용하지말자**
...
- ### `Singleton` vs `static class`
    ...
    > 인스턴스화 하고 접근할 것인가? 
    >
    > 객체를 단 한개만 생성하고 사용할 것인가
    >
    > https://javarevisited.blogspot.com/2013/03/difference-between-singleton-pattern-vs-static-class-java.html
    >
    > https://codewala.net/2016/10/16/singleton-vs-static-class-key-differences-and-usages/

    - **`Single ton`**
        - 로드되기 전까지는 메모리 낭비를 줄일 수 있다
        - 상태를 저장하거나 유지해야한다면 싱글톤이 적절할수 있다.
        - 생성자에 만약 매개변수가 필요하다면 싱글턴을 구현해야한다.
        - 매개변수로 전달할 경우가 있다면?? Static 은 매개변수로써는 활용할 수 없다.
        - 싱글턴은 그래서 다형성을 구현할 수 있다. DI도 활용가능!
        - 인터페이스 구현이 가능합니다
    - **`static`**
        - 사용되지 않더라도 메모리를 차지한다.
        - 생성자 호출 시점 제어가 불가능하다.
        - 힙을 벗어난 영역에 저장되어, 메모리관리 대상에서 제외되므로, 소멸이 필요한 객체라면 싱글턴을 고민해보자
        - Struct 로 구현이 충분히 가능하다.
        - 메소드 호출이 더더욱 빠르다 - static 바인딩은 컴파일시 일어난다
...
- ###`Codable` vs `NSCoding`
  ...
  - Codable
    ...
    - 쉽게 JSON Serialization 가능

    - Swift 객체에만 해당되는 프로토콜
...
- #### 캡슐화 왜 해야돼?
  ...
  - 데이터를 `mutating`하는 건 오퍼레이터에서만 하게하자 - 객체는 자율적이기에 객체에 메시지를 보내서 해야한다.
