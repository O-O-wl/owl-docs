...
- #### `struct` vs `class`
  ...
  - **struct**
    - 객체들사이의 참조 관계를 단순화하는 것 충분히 가능할 때
    - 만약 멤버로 class 타입을 써야한다면 값이라는 정의에 어긋날 수 있다. 
  - **class**
    - 데이터의 양이 복사하기에는 좀 크지 않은가?  그런 경우는 복사보다는 참조가 나을수있다. COW를 취하지 않는 경우
    -  공유자원으로서의 역할을 하는 객체일 때
...
- #### `if let`  vs `guard let` 
  ...
  - **guard**
    ...
    - 한 방향으로의 의도가 존재하고 실패했을 때 하는 일을 명시

    - guard 뭔가 성공해야 한다는 걸 좀 더 명시 할 수 있다.

    - else 는 의도와 다른 일을 할 때 해야할 일 , 즉 실패에 대한 핸들링이다
  ...
  - **if let**
    ...
    - 양방향 모두 동등한 무게를 가질 때 ,  즉 단순한 2가지 혹은 그 이상의 분기를 표현한다.
    - else 는 if와 다른 조건일 때, 즉 다른 경우에 대한 핸들링 이다.
    - 둘 다 정답이 될 수 있는 맞는 경우이다.
